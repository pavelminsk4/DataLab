...
from .filling_social_top_languages_widget import social_top_languages_widget_image
                ...
                for p in cell.paragraphs:
                    if '$EXPORT_TITLE$' in p.text:
                        p.text = p.text.replace(
                            '$EXPORT_TITLE$', export_title(item.module_project_id))
                    if '$EXPORT_PERIOD$' in p.text:
                        p.text = p.text.replace(
                            '$EXPORT_PERIOD$', export_period(item.module_project_id))
                    if '$EXPORT_TOC$' in p.text:
                        p.text = p.text.replace('$EXPORT_TOC$', ' ')
                        if item.soc_summary:
                            run = cell.add_paragraph(
                                '').add_run('Report Summary')
                            font_one(run, cell)
                        ...
                        if item.soc_content_volume or item.soc_content_volume_by_top_locations or item.soc_content_volume_by_top_authors or item.soc_content_volume_by_top_languages:
                            run = cell.add_paragraph(
                                '').add_run('Potential Reach')
                            font_one(run, cell)
                            if item.soc_content_volume:
                                run = cell.add_paragraph().add_run(proj.social_widgets_list.content_volume.title +
                                                                   ' (per ' + proj.social_widgets_list.content_volume.aggregation_period + ')')
                                font_two(run, cell)
                            if item.soc_content_volume_by_top_locations:
                                run = cell.add_paragraph().add_run(proj.social_widgets_list.content_volume_by_top_locations.title +
                                                                   ' (per ' + proj.social_widgets_list.content_volume_by_top_locations.aggregation_period + ')')
                                font_two(run, cell)
                            if item.soc_content_volume_by_top_authors:
                                run = cell.add_paragraph().add_run(proj.social_widgets_list.content_volume_by_top_authors.title +
                                                                   ' (per ' + proj.social_widgets_list.content_volume_by_top_authors.aggregation_period + ')')
                                font_two(run, cell)
                            if item.soc_content_volume_by_top_languages:
                                run = cell.add_paragraph().add_run(proj.social_widgets_list.content_volume_by_top_languages.title +
                                                                   ' (per ' + proj.social_widgets_list.content_volume_by_top_languages.aggregation_period + ')')
                                font_two(run, cell)
                            cell.add_paragraph()
