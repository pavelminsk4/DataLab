...
def best_times_to_post(pk, widget_pk):
    ...
    for day_of_week in range(1, 8):
        for hour in range(0, 24):
            ...
            results.append({
                        'date': posts_of_day.first().date.strftime('%b %d, %Y %I:%M %p') if posts_of_day.first() else None,
                        ...
                        'reply': posts_of_day.aggregate(Sum('count_replies'))['count_replies__sum'] if posts_of_day.count() else 0})
