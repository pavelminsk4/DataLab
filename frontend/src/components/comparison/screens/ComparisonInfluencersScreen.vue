...
export default {
  ...
  computed: {
    ...mapState(['influencers']),

    selectedWidgets() {
      if (!this.influencers.widgets.length) return
      return comparisonWidgetsList[this.projectsModule].influencers
        .map((widget) => {
          const foundWidget = this.influencers.widgets.find(
  ...
  created() {
    this[action.GET_INFLUENCERS_WIDGETS](this.currentProject.id)
  },
  methods: {
    ...mapActions([action.GET_INFLUENCERS_WIDGETS]),
  },
}
</script>
