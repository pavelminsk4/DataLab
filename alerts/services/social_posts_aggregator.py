...
def social_posts_aggregator(project_id):
    ...
    if country_dimensions:
        posts = posts.filter(reduce(lambda x, y: x | y, [Q(locationString=x) for x in country_dimensions]))
    if language_dimensions:
        posts = posts.filter(reduce(lambda x, y: x | y, [Q(language=x) for x in language_dimensions]))
    if source_dimensions:
        posts = posts.filter(reduce(lambda x, y: x | y, [Q(source=x) for x in source_dimensions]))
    if author_dimensions:
        posts = posts.filter(reduce(lambda x, y: x | y, [Q(user_name=x) for x in author_dimensions]))
    if sentiment_dimensions:
        posts = posts.filter(reduce(lambda x, y: x | y, [Q(sentiment=x) for x in sentiment_dimensions]))
