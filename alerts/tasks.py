...
def alert_sender():
    ...
    for alert in alerts:
        ...
        for item in items:
            delta = check_new_posts(item, alert.triggered_on_every_n_new_posts)
            if delta >= alert.triggered_on_every_n_new_posts:
                if item.module_type == 'Project':
                    posts = posts_aggregator(item.module_project_id).order_by('-creationdate')[:alert.how_many_posts_to_send]
                if item.module_type == 'ProjectSocial':
                    posts = social_posts_aggregator(item.module_project_id).order_by('-created_at')[:alert.how_many_posts_to_send]
                users = alert.user.all()
                mails_list = list(users.values_list('email', flat=True))

                msg = EmailMessage()
                msg['Subject'] = f'''Your '{alert.title}' alert from the Datalab project has been triggered.'''
                msg['From'] = 'Datalab'
                msg['To'] = mails_list
