    ...
    for item in items:
      ...
      if delta >= alert.triggered_on_every_n_new_posts:
        if item.module_type == 'Project':
          posts = posts_agregator(item.module_project_id).order_by('-creationdate')[:alert.how_many_posts_to_send]
        if item.module_type == 'ProjectSocial':
          posts = social_posts_agregator(item.module_project_id).order_by('-created_at')[:alert.how_many_posts_to_send]
