...
class ComparisonSocialDemographyTests(APITestCase):
    ...
    def test_demography_feature_social(self):
        ...
        self.assertEqual(json.loads(response.content)[1]['widget_name'], 'top_keywords_by_location')
        self.assertEqual(json.loads(response.content)[2]['widget_name'], 'authors_by_location')
        self.assertEqual(json.loads(response.content)[3]['widget_name'], 'top_languages_by_location')
        self.assertEqual(json.loads(response.content)[4]['widget_name'], 'sentiment_by_locations')
        self.assertEqual(json.loads(response.content)[5]['widget_name'], 'authors_by_gender')
        self.assertEqual(json.loads(response.content)[6]['widget_name'], 'top_gender_by_location')
