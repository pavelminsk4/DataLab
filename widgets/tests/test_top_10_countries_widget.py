...
class Top10CountriesWidgetTests(APITestCase):
  def test_response_list(self):
    ...
    post7 = Post.objects.create(feedlink=flink4, entry_title='5 post title', feed_language=sp, entry_published=datetime(2023, 9, 3, 6, 37), entry_author='AFP')
    post8 = Post.objects.create(feedlink=flink4, entry_title='6 post title', feed_language=sp, entry_published=datetime(2023, 9, 3, 6, 37), entry_author='')
    ...
    widget_pk = pr1.widgets_list_2.top_10_countries_widget_id
    url = reverse('widgets:top_10_countries_widget', kwargs={'pk':pr1.pk, 'widget_pk':widget_pk})
    ...
    widget_pk = pr2.widgets_list_2.top_10_countries_widget_id
    url = reverse('widgets:top_10_countries_widget', kwargs={'pk':pr2.pk, 'widget_pk':widget_pk})
    response = self.client.get(url)
    self.assertEqual(response.status_code, status.HTTP_200_OK)
    res2 = [
