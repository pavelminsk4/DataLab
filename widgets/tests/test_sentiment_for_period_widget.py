...
class SentimentForPeriodWidgetTests(APITestCase):
  def test_response_list(self):
    ...
    post3 = Post.objects.create(feedlink=flink2, entry_title='Third post title', feed_language=sp1, entry_published=datetime(2021, 10, 3, 6, 37), entry_author='AFP', sentiment='negative', summary_vector=[])
    post4 = Post.objects.create(feedlink=flink2, entry_title='4 post title', feed_language=sp2, entry_published=datetime(2021, 9, 3, 6, 37), entry_author='AFP', sentiment='negative', summary_vector=[])
    post5 = Post.objects.create(feedlink=flink3, entry_title='5 post title', feed_language=sp2, entry_published=datetime(2022, 9, 3, 6, 37), entry_author='AFP', sentiment='positive', summary_vector=[])
    post6 = Post.objects.create(feedlink=flink3, entry_title='6 post title', feed_language=sp2, entry_published=datetime(2022, 10, 3, 6, 37), entry_author='EFE', sentiment='neutral', summary_vector=[])
    ...
    url = reverse('widgets:onl_sentiment_for_period', kwargs={'pk':pr.pk, 'widget_pk':widget_pk})
    data = {
