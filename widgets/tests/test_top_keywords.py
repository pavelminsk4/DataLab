...
class TopKeywordsTests(APITestCase):
  def test_top_keywords_api(self):
    ...
    Post.objects.create(feedlink=flink1, entry_title='Second post title', feed_language=sp1, entry_published=datetime(2022, 9, 3, 6, 37), entry_author='AFP', sentiment='neutral',
                        entry_summary = 'the keyword text field',)
    Post.objects.create(feedlink=flink2, entry_title='Third post title', feed_language=sp1, entry_published=datetime(2023, 9, 3, 6, 37), entry_author='AFP', sentiment='negative',
                        entry_summary = 'keyword uno два')
    Post.objects.create(feedlink=flink2, entry_title='Third post title', feed_language=sp1, entry_published=datetime(2023, 9, 3, 6, 37), entry_author='AFP', sentiment='negative',
                        entry_summary = 'the keyword uno field')
