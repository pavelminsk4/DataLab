...
class DepartmentAlertsTests(APITestCase):
    def test_department_alerts_endpoint(self):
        ...
        Alert.objects.create(title='Test Alert', department=dep2)
        Alert.objects.create(title='Weekly Alert', department=dep2)
        Alert.objects.create(title='Hourly Alert', department=dep2)
