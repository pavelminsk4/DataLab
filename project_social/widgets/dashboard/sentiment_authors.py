...
def calculate_for_sentiment_authors(posts, aggregation_period, top_counts):
  ...
  results = {author: list(posts.filter(user_name=author).annotate(date_trunc=trunc_function('date', aggregation_period)).values('sentiment').annotate(sentiment_count=Count('sentiment')).order_by('-sentiment_count')) for author in top_authors}
