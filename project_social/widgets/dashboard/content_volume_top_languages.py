...
def content_volume_top_languages(request, pk, widget_pk):
    ...
    res = calculate_for_content_volume_top_languages(posts, widget.aggregation_period, widget.top_counts)
...
def to_csv(request, pk, widget_pk):
    posts, widget = project_posts_filter(pk, widget_pk)
    result = calculate_for_content_volume_top_languages(posts, widget.aggregation_period, widget.top_counts)
    dates = [str(elem['date']) for elem in list(*result[0].values())]
    fields = ['Language'] + dates
