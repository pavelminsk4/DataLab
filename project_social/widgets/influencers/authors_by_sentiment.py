...
def authors_by_sentiment(pk, widget_pk):
    ...
    return JsonResponse(calculate_for_authors_by_sentiment(posts, widget), safe = False)

def authors_by_sentiment_report(pk, widget_pk):
    posts, widget = project_posts_filter(pk, widget_pk)
    return {
        'data': calculate_for_authors_by_sentiment(posts, widget),
        'widget': {'authors_by_sentiment': model_to_dict(widget)}
    }

def calculate_for_authors_by_sentiment(posts, widget):
