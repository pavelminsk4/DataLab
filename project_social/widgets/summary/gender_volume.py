...
def post_agregator_gender_volume(posts, aggregation_period, top_counts):
  ...
  results = [{name: list(posts.filter(user_gender=name).annotate(date=Trunc('creation_date', aggregation_period)).values("creation_date").annotate(created_count=Count('id')).order_by("date"))} for name in top_authors]
  dates = set()
