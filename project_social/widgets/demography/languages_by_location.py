...
def calculate(posts, widget):
    top_countries = [i['locationString'] for i in posts.values('locationString').annotate(language_count=Count('language', distinct=True)).order_by('-language_count')[:widget.top_counts]]
    results = {}
