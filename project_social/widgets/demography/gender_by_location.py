...
def calculate_for_gender_by_location(posts, top_counts):
    top_locations = posts.values('country').annotate(posts_count=Count('id')).order_by('-posts_count').values_list('country', flat=True)[:top_counts]
    results = {location: {'male': 0, 'female': 0, 'undefined': 0} for location in list(top_locations)}
    for post in posts:
        if results.get(post.country):
            results[post.country][post.user_gender] += 1
    return results
