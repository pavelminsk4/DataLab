...
def calculate_for_authors_by_location(posts, top_counts):
    results = list(posts.values('country').annotate(user_count=Count('user_alias', distinct=True)).order_by('-user_count')[:top_counts])
    top_countries = [i['country'] for i in posts.values('country').annotate(author_count=Count('user_alias', distinct=True)).order_by('-author_count')[:5]]
    results = []
    for country in top_countries:
        top_authors = posts.filter(country=country).values('user_alias').annotate(posts_count=Count('id')).order_by('-posts_count')[:5]
